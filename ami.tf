...
resource "null_resource" "ansible-apply" {
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "ansible-pull -i localhost, -U https://github.com/zeeshan1203/ansible.git roboshop-pull.yml -e ENV=ENV  -e COMPONENT=${var.COMPONENT} -e PAT=${jsondecode(data.aws_secretsmanager_secret_version.secrets.secret_string)["PAT"]} -e APP_VERSION=${var.APP_VERSION} &>/tmp/out"
    ]
  }
}
